...
class TacticalViewDrawer:
    ...
    def draw(
        ...
        for frame_idx, frame in enumerate(video_frames):
            ...
            for keypoint_index, keypoint in enumerate(tactical_court_keypoints or []):
                x, y = keypoint
                x += self.start_x
                y += self.start_y
            ...
            if (
                ...
                for player_id, position in frame_positions.items():
                    # Get player's team
                    team_id = frame_assignments.get(
                        player_id, 1
                    )  # Default to team 1 if not assigned
                    ...
                    x, y = (
                        int(position[0]) + self.start_x,
                        int(position[1]) + self.start_y,
                    )
